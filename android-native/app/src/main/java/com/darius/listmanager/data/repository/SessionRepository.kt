...
class SessionRepository(
    ...
    suspend fun addOrIncrementItem(sessionId: Long, productId: Long, quantity: Int = 1) {
        ...
        try {
            ...
            Log.w(TAG, "addSessionItem failed: ${e.javaClass.simpleName}")
        }
    }
    ...
    suspend fun setItemQuantity(sessionId: Long, productId: Long, quantity: Int) {
        ...
        try {
            ...
            Log.w(TAG, "setItemQuantity failed: ${e.javaClass.simpleName}")
        }
    }

    suspend fun deleteItem(itemId: Long) {
        sessionItemDao.deleteById(itemId)
        // Negative ids are locally-created rows that never reached the server.
        if (itemId <= 0) return
        try {
            ...
            Log.w(TAG, "deleteSessionItem failed: ${e.javaClass.simpleName}")
        }
    }
    ...
    suspend fun clearSession(sessionId: Long) {
        ...
        try {
            ...
            Log.w(TAG, "clearSession failed: ${e.javaClass.simpleName}")
        }
    }
